...
describe('Balustrade — the Overlook #00057', () => {
    ...
    it('does not offer a third Claude patrol session', () => {
        const wrapper = mount(Balustrade);
        expect(wrapper.find('[data-test="balustrade-tc"]').exists()).toBe(false);
        expect(wrapper.find('[data-patrol-lamp]').exists()).toBe(false);
        expect(wrapper.get('[data-dispatch-trigger]').text()).toBe('Brief ▾');
    });
