...
<template>
    <section class="border-t border-wb-edge-soft bg-wb-canvas/40 px-5 py-3">
        <div class="wb-stamp-label mb-2">Review</div>
        <textarea
            v-model="body"
            class="wb-input w-full text-[12px] min-h-[4.5rem] resize-y"
            placeholder="Leave a note — required for Comment and Request Changes."
            :disabled="drydock.submitting.value"
            data-test="review-body"
        />
        <p v-if="error" class="text-wb-pulse-crashed font-mono text-[11px] mt-2" data-test="review-error">
            {{ error }}
        </p>
        <div class="flex items-center gap-2 mt-3">
            ...
            <span v-if="drydock.submitting.value" class="text-wb-text-faint font-mono text-[11px] ml-auto">
                Sending…
            </span>
        </div>
    </section>
