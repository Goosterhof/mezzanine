...
│   │   │   └── substrate.rs .. CommandBuilder for wsl.exe (Windows) / bash (Unix); SessionSpec::for_target + for_crier (#00060) + env injection (export K=V before exec)
...
│   │   ├── chronicle/           Per-scientist JSONL transcript writer + reader (tail) + path migration
│   │   │   ├── mod.rs ........ Module root, re-exports
│   │   │   ├── writer.rs ..... ChronicleWriter — base-dir holder + pause flag (vestigial; see commit log)
│   │   │   ├── reader.rs ..... ChronicleReader — per-scientist JSONL tail (Arc 2); push-always, initial offset = file length so a resurrected scientist does not replay history (0 only for a new file)
...
│   │   ├── crier/                 The Crier's Watch (#00060) — relay watch-post backend
│   │   │   └── mod.rs ........ Token-file reader (~/.config/zmuuzn/town-crier-token) + CrierWatchState / CrierQueueEntry / CrierStatus serde shapes
│   │   └── commands/
...
│   │       ├── crier.rs ...... dispatch_crier / recall_crier / read_crier_watch_state — The Crier's Watch (#00060)
...
| The Field Journal (#00059) | The floor becomes a page — the pixel engine retired wholesale. J-1: `SketchPen` + `drawScientist`/`figureHeadTop` ported from the ratified Atelier mock into `src/observer/{pen,figure}.ts` (TS, `mulberry32` inlined, Atelier proportions exported, ghosts default OFF in production). J-2: `scene.js` renderer swapped — boiling ink at `BOIL_HOLD = 14` over a 60fps walk skeleton, pre-rendered paper blit (full-DPR, 1:1; pixel-era `imageSmoothingEnabled=false` retired with the tiles), four ink stations as permanent fixtures at their POSITIONS anchors, simpler ink minions; controller API unchanged. J-3: Caveat enters the gadget (self-hosted woff2 + OFL, `document.fonts.load` guard before RAF), canvas margin captions (state dot, target, mission · elapsed, crash voice + idle warn verbatim from the voice lock), `[ recall ]` canvas hit-region riding the new `recallScientist:<id>` wire action in `projection.ts`, canvas empty voice; the DOM nameplate railing + `ScientistRow` + `LabFloor`'s empty overlay retired (RecentlyRecalledStrip re-homed under the Balustrade). J-4: pencil plumb-line + sketched nail (PENCIL from the pen), `TornPaperEdge.vue` (fill = PAPER), final strip dressing (figures only, on paper). Signature: the selected figure renders with pencil construction ghosts — being selected is being drawn. Gift: ALL typefaces self-host (Space Grotesk + JetBrains Mono CDN links removed — the gadget keeps its hands offline). | ✅ Static-green 2026-06-13 — oxlint 0 errors, oxfmt clean, vue-tsc clean, vitest 522/522 (net +23 on the 499-spec pre-arc suite: pen 18 + figure 26 + TornPaperEdge 4 + recall wire 5 + canvas-voice 3 new; plate/row specs retired with their components), v8 coverage 97.22% lines / 92.99% branches / 96.02% functions / 97.22% statements, vite build clean (ink scene chunk 20.81 kB — down from the pixel engine's 37 kB). No Rust changes; `cargo check` deferred to Windows. **Runtime-witnessed same day** on the Windows-with-WSL2 host (real Chromium against the vite server, IPC stubbed at the `__TAURI_INTERNALS__` seam, every invoke recorded): §9 items 1–11 evidenced in #00059 §8 with screenshots at `documents/screenshots/00059-*` — ghosts + ghost transfer pixel-diffed under a frozen boil, canvas `[ recall ]` → `recall_scientist` IPC recorded with zero DOM nameplate ghosts, **mandatory perf gate green: 6 scientists + ghosts, 59.96fps, max frame 16.8ms, 0 frames >25ms over 30s — no fallback needed**. The gate also caught and fixed a light-pool staleness wound (`LabFloor.schedulePoolRecompute`, +1 spec → vitest 523/523, coverage 97.22/92.92/96.04/97.22). **DELIVERED 2026-06-13** — the investor reviewed the §8 harness evidence + screenshots and accepted the autonomous-harness witness as sufficient for this frontend-only arc (no Rust changed; IPC stubbed); the Delivery Seal was applied by hand after the task-master close-out parser thrice false-halted on a clean Drill Sergeant report. One accepted gap carried to #00059 §13: confirm a *live* pty dispatch leaves the floor on a real `recall_scientist` at the next natural boot. |
| The Crier's Watch (#00060) | The patrol never sleeps — the town-crier review-bus relay auto-arms on every Mezzanine boot as a singleton **ephemeral** crier scientist (infrastructure, not a roster mission), and reports to a new **TC** watch-post drawer on the Balustrade. Phase 1 (Substrate): `SessionSpec` gains `env: Vec<(String, String)>` + a `for_crier` constructor (`LabRoot` cwd, `--dangerously-load-development-channels server:town-crier-relay` args, `TC_RELAY_ARMED=1` + `TOWN_CRIER_LAB_TOKEN` env — NOT `TC_RELAY_TOKEN`, which `.mcp.json` overwrites); token-file reader at `~/.config/zmuuzn/town-crier-token`; singleton double-arm guard via `crier_scientist_id`; new `crier/mod.rs` + `commands/crier.rs` (`dispatch_crier` / `recall_crier` / `read_crier_watch_state`). Phase 2 (Watch panel): `'criers-watch'` (TC) + `PatrolLamp` on the Balustrade; `src/crier/` slice; live `GET /open` bus status. Phase 3 (Auto-arm + watch glass): `armOnBoot()` in `App.vue` gated on wizard clearance; embedded xterm bound to the crier PTY event stream; Arm / Stand Down / Take a turn. Gift: `PatrolLamp.vue` — amber heartbeat off parsed `[crier-doorbell] pushed turn` lines. | ✅ DELIVERED 2026-06-22 (PR #35, squash-merged) — reviewed by the general (Approve-worthy, 0 blockers) and the Tribunal (NEEDS-WORK → fixed: the ephemeral crier leaked into the recall strip on Stand Down `roster/manager.rs`; `read_crier_watch_state` dropped the session id so a reopened panel rendered ON PATROL over a dead glass — now returns + binds it via the new `CrierWatchState.scientistId`; queue row shows the real PR # parsed from `prUrl`, not the bus request id). Gates green: cargo 236 + `clippy -D warnings`, vitest 573/573 (+ crier specs), vue-tsc, oxlint 0 errors, oxfmt, vite build. **Carried gap:** runtime witness on the Windows+WSL2 host (the live relay actually nudging a session with the token file present) is NOT yet done — static-only delivery; cf. #00059's accepted-harness-witness precedent, but here Rust changed, so a live boot witness remains pending. Experiment log: `documents/experiment-logs/00060-the-criers-watch.md`. |
...
- Latest delivered arc log: `documents/experiment-logs/00060-the-criers-watch.md` (The Crier's Watch — the patrol never sleeps; DELIVERED 2026-06-22, static-green; runtime witness carried)
- Previous arc log: `documents/experiment-logs/00059-the-field-journal.md` (The Field Journal — the floor becomes a page; DELIVERED 2026-06-13, investor accepted the harness witness)
- Previous arc log: `documents/experiment-logs/00057-the-overlook.md` (The Overlook — two-storey frame, DELIVERED 2026-06-12)
